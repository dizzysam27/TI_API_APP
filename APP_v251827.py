...
class MouserLoginPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # CREATE ENTRY BOXES FOR THE CLIENT ID AND CLIENT SECRET. ALSO CREATE A BUTTON WHICH RETRIEVES AN ACCESS TOKEN WHEN PRESSED
        self.username_entry = customtkinter.CTkEntry(self, width=200, placeholder_text="Mouser ID",bg_color='#9d8ad4')
...
class DigiKeyLoginPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # CREATE ENTRY BOXES FOR THE CLIENT ID AND CLIENT SECRET. ALSO CREATE A BUTTON WHICH RETRIEVES AN ACCESS TOKEN WHEN PRESSED
        self.username_entry = customtkinter.CTkEntry(self, width=200, placeholder_text="Mouser ID",bg_color='#222222',corner_radius=50)
        self.username_entry.place(relx=0.5, rely=0.35, anchor='n')
        self.password_entry = customtkinter.CTkEntry(self, width=200, show="*", placeholder_text="Client Secret",bg_color='#222222',corner_radius=50)
        self.password_entry.place(relx=0.5, rely=0.43, anchor='n')
...
class SearchPage(navigation_side_bar):
    ...
    def __init__(self, parent, controller):
        ...
        self.search_frame_search_button = customtkinter.CTkButton(self.blank_frame, text="Search",command=self.search_event, compound="top", fg_color="#CC0000")
        ...
        self.part_product_information_frame = customtkinter.CTkScrollableFrame(self.blank_frame, label_text="Product Information", label_fg_color = "#CC0000", label_text_color = 'white')
        ...
        quality_estimator_button = customtkinter.CTkButton(self.blank_frame, text="Quality Estimator",command=self.quailty_estimator_event, compound="top", fg_color="#CC0000")
        ...
        self.info_table = CTkTable(master=self.part_product_information_frame, row = 30 , column =2 ,values = '', wraplength = '110')
        ...
        iteration = 0
        self.not_checkboxes_list=["Url", "Datasheet", "Quality Estimator Url", "Id", "Description"]
        for i in range(35):
            ...
            if navigation_side_bar.info_types[i] in self.not_checkboxes_list:
                ...
                switch = customtkinter.CTkCheckBox(master=product_information_frame, text=self.info_types[i], command=lambda f=iteration: self.switchable_button_press(f), fg_color="#CC0000")
                switch.select()
    ...
    def enable_all_filters(self):
        ...
        if (self.master_var.get() ==1):
            ...
            for i in range(2, 35):
                ...
                info_text = navigation_side_bar.info_types[i]
                # IGNORES URLs AS THEY HAVE SEPERATE BUTTON
                if info_text == "Url" or info_text == "Datasheet" or info_text == "Quality Estimator Url":
                    pass
                
                # DISPLAYS DATA IN TABLE
                else:
                    # SELECTS FILTER SWITCHES
                    self.product_information_frame_switches[count-2].select()
                    self.switch_state[count-2]=1
                    
                    # TRIES FINDING INFO IN DICTIONARY
                    try:
                        self.info_table.insert(count-2, 0, info_text, text_color = 'black')
                        textDisplay = str( self.called_info[navigation_side_bar.info_get[i]] )
                        self.info_table.insert(count-2, 1, textDisplay, text_color = 'black')
                        
                    # IF NO INFO IN DICTIONARY, PRINT N/A IN DIFFERENT COLOUR
                    except KeyError:
                            self.info_table.insert(count-2, 0, info_text, text_color = '#ad9d05')
                            textNothing = "Information not available"
                            self.info_table.insert(count-2,1, textNothing, text_color = '#ad9d05')
                    count +=1
    ...
    def switchable_button_press(self, number):
        ...
        for j in range(len(self.switch_state)-position):
            self.info_table.edit_row(j+position, value = '')
...
class SavedFrame(navigation_side_bar):
    ...
    def __init__(self, parent, controller):
        ...
        # self.part_product_information_frame.bind('<Motion>', self.display_saved_info)
        ...
        self.b3 = customtkinter.CTkButton(self.blank_frame, text = 'Display', command = self.display_saved_info)
        self.b3.grid(row=1, sticky ='nw')
